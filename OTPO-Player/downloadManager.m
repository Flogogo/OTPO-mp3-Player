...
-(void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data{
    ...
    for (int i = 0; i < [self.arrayOfTrack count]; i++) {
        if ([[[connection originalRequest] URL] absoluteString] == [self.arrayOfTrack[i] urlPicture])
        {
...
-(void)startDownload{
        for (int i = 0; i < [self.arrayOfTrack count]; i++){
        NSURLRequest *theRequest = [NSURLRequest  requestWithURL:[NSURL URLWithString:[self.arrayOfTrack[i] urlPicture]]
